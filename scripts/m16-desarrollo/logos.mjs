/**
 * Escudos del Torneo Nacional Desarrollo M16: de los originales de Recursos a
 * `public/`, redimensionados.
 *
 *   node scripts/m16-desarrollo/logos.mjs --plan
 *   node scripts/m16-desarrollo/logos.mjs --execute
 *
 * Van como ARCHIVO y la base guarda la RUTA — nunca base64, que es lo que infló
 * `clubs` a 905 escudos embebidos.
 *
 * Seis de los nueve escudos son la misma imagen de unión que ya usa el
 * Argentino Juvenil (URBA, UROBA, Entre Ríos, Mar del Plata, Austral, Alto
 * Valle). Se escribe un archivo por club igual: la convención de
 * `public/clubs/` es un archivo por id y 20 KB no justifican romperla.
 *
...
import { CLUBES, LOGO_TORNEO } from './datos.mjs';

const REPO = process.cwd();
const ORIGEN = 'C:/Users/srome/OneDrive/Documentos/________S22/Recursos/ARGENTINA/SELECCIONADOS UNIONES/WEB';
const ORIGEN_TORNEO = 'C:/Users/srome/OneDrive/Documentos/________S22/Recursos/M16-Logo.png';
const LADO = 256;
 ...
 * color propio.
 *
 * `stats().dominant` de sharp devuelve el fondo en estos PNG, que traen blanco
 * opaco. Se descarta entonces el blanco, el negro y el gris (saturación baja),
 * y se cuenta sobre lo que queda, cuantizado a 16 niveles por canal para que
 * dos píxeles casi iguales caigan en el mismo balde. Un escudo en blanco y
 * negro —el de Chubut lo es— devuelve `null` a propósito.
 */
...
async function main() {
  ...
  for (const club of CLUBES) {
    const origen = resolverArchivo(club.escudo);
    const destino = path.join(destinoClubes, `${club.id}.png`);
    ...
    console.log(`  ${club.id.padEnd(20)} ← ${club.escudo.padEnd(22)} ${String(Math.round(pesoOrigen / 1024)).padStart(4)} KB → ${String(Math.round(pesoDestino / 1024)).padStart(3)} KB · ${colores[club.id] ?? '— blanco y negro —'}`);
  }

  const destinoTorneo = path.join(REPO, 'public', LOGO_TORNEO.replace(/^\//, ''));
  const pesoTorneo = await procesar(ORIGEN_TORNEO, destinoTorneo);
  console.log(`\n  ${path.basename(LOGO_TORNEO)} ← M16-Logo.png ${String(Math.round(fs.statSync(ORIGEN_TORNEO).size / 1024)).padStart(4)} KB → ${Math.round(pesoTorneo / 1024)} KB`);

  console.log(`\ntotal escudos: ${Math.round(antes / 1024)} KB → ${Math.round(despues / 1024)} KB`);

  const salida = path.join(REPO, 'scripts', 'm16-desarrollo', 'colores.json');
  if (modo === 'execute') {
