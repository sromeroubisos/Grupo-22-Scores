-- Repair environments where tournament priority was dropped by schema simplification
-- but the later restoration migration was not applied.
...
BEGIN
    ...
    WHERE p_include_hidden OR COALESCE(t.is_visible, true) = true
    ORDER BY
