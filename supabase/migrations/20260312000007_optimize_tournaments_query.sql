-- ============================================================
-- Optimized Global Tournaments Centralized Architecture (v2)
-- ============================================================
...
BEGIN
    ...
    -- Priority: Explicit parameter > auth.uid()
    v_actual_user_id := COALESCE(p_viewer_user_id, auth.uid());

    RETURN QUERY
    WITH follow_stats AS (
        -- Optimize follower counting and user-follow status
        SELECT 
            tf.tournament_id, 
            COUNT(*) as count,
