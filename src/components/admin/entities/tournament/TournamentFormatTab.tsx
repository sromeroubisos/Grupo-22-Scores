...
export function TournamentFormatTab({ data, id }: { data?: TournamentRow; id?: string; matchCount?: number }) {
    ...
    return (
        <div className="flash-ui-container dark bg-transparent format-console-shell" style={{ '--accent': '#00a365', minHeight: 'auto' } as CSSProperties}>
            ...
            <section className="tournament-format-mobile" aria-label="Formato del torneo">
                <article className="tsm-card tsm-card-state">
                    <div className="tsm-card-eyebrow">Formato de eventos</div>
                    ...
                    <div className="tsm-participants-counters">
                        <div className="tsm-counter">
                            <span className="tsm-counter-label">Eventos</span>
                            <span className="tsm-counter-value">{definitions.length}</span>
                        </div>
                        <div className="tsm-counter">
                            <span className="tsm-counter-label">De puntuacion</span>
                            <span className="tsm-counter-value">{scoringEvents.length}</span>
                        </div>
                        <div className="tsm-counter">
                            <span className="tsm-counter-label">Categorias</span>
                            <span className="tsm-counter-value">{categoryCount}</span>
                        </div>
                        <div className="tsm-counter">
                            <span className="tsm-counter-label">Puntos combinados</span>
                            <span className="tsm-counter-value">{totalScoringPoints}</span>
                        </div>
                    </div>
                </article>

                <article className="tsm-card">
                    <div className="tsm-card-eyebrow">Eventos disponibles</div>
                    {definitions.length === 0 ? (
                        <div className="tsm-next-empty" style={{ padding: '8px 0' }}>
                            <strong>No hay eventos configurados</strong>
                            <small>Agrega un primer evento para que la consola de partido pueda cargar acciones.</small>
                        </div>
                    ) : (
                        <ul className="tsm-phase-list">
                            {definitions.slice(0, 12).map((definition, index) => (
                                <li key={`${definition.type}-${index}`} className="tsm-phase-item">
                                    ...
                                    <div className="tsm-phase-text">
                                        ...
                                        <small>
                                            {CATEGORY_OPTIONS.find((c) => c.value === definition.category)?.label || definition.category}
                                            {definition.points ? ` · ${definition.points} pts` : ''}
                                        </small>
                                    </div>
                                </li>
                            ))}
                    ...
                    {definitions.length > 12 ? (
                        <p style={{ fontSize: 12, color: 'var(--text-dim, #6b7280)', marginTop: 10, textAlign: 'center' }}>
                            Mostrando primeros 12 de {definitions.length}.
                        </p>
                    ) : null}
                </article>
                ...
                <article className="tsm-card">
                    ...
                    <div className="tsm-action-grid">
                        <button
                            type="button"
                            className="tsm-action-btn primary"
                        ...
                        <button
                            type="button"
                            className="tsm-action-btn"
                            onClick={resetDefaults}
                            disabled={isApiManaged}
                        >
                            <RotateCcw size={16} /> Restaurar preset
                        </button>
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-dim, #6b7280)', marginTop: 10, lineHeight: 1.45 }}>
                        Estos eventos se sincronizan con la consola de gestion de partido.
                    </p>
                </article>
                ...
                {isApiManaged && (
                    <article className="tsm-card" style={{ borderColor: 'rgba(96, 165, 250, 0.4)', background: 'rgba(96, 165, 250, 0.06)' }}>
                        ...
                        <p style={{ fontSize: 12, color: 'var(--text-secondary, #b6bdcc)', lineHeight: 1.5, margin: 0 }}>
                            Los eventos vienen del proveedor externo. Edicion deshabilitada.
                        </p>
                    </article>
                )}
            </section>
