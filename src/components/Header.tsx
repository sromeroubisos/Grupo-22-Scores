...
export default function Header() {
    ...
    const clubRoles = ['admin_club'];
    const coachRoles = ['entrenador'];
    const adminRoles = ['admin_general', 'admin_union', 'admin_torneo', 'operador', ...clubRoles, ...coachRoles];
                    ...
                    <div className="g22-user-wrapper" ref={menuRef}>
                        ...
                        <div className={`g22-user-menu ${isUserMenuOpen ? 'show' : ''}`} style={isUserMenuOpen ? { opacity: 1, visibility: 'visible', transform: 'translateY(0)' } : {}}>
                            ...
                            {user && adminRoles.includes(user.role) && (
                                <Link
                                    href={coachRoles.includes(user.role) ? "/entrenador" : clubRoles.includes(user.role) ? "/club-admin" : "/admin"}
