...
import { hrefParaClubes, MODULOS, PROMO } from '@/content/para-clubes';
import styles from './ClubsPromoCard.module.css';

/**
 * La placa de "G22 para clubes", en sus dos tamaños.
 *
 ...
 * pasos y pide la demo. Ojo con dónde se ve: `.sidebarRight` de la home se
...
export default function ClubsPromoCard({ variant }: Props) {
    ...
    const alHacerClick = () => {
        trackEvent('clubs_promo_click', { location: origen });
    };
    ...
    return (
        ...
        >
            <Link href={hrefParaClubes(origen)} className={styles.link} onClick={alHacerClick}>
                ...
                <span className={styles.accion}>
                    {esSidebar ? PROMO.accionSidebar : PROMO.accionFeed}
                    <svg
