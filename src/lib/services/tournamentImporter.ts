...
const upsertTournament = async (t: Tournament, sportId: SportId, results: any) => {
    ...
    try {
        // B15: `tournaments.id` is a UUID generated by the DB — static slugs like
        // t.id must NOT be written there. The slug lives in `external_id`
        // (TEXT UNIQUE, see migration 20260218140100_add_external_id_columns.sql)
        // and the upsert conflicts on it, letting the DB mint the UUID PK.
        const payload = {
            external_id: t.id,
            name: t.name,
            sport_id: sportId,
