...
// Cache configuration
const CACHE_TTL_MATCHES = 60; // 1 minute for match lists
...
// Consolidated import format for the API response types based on inspection
// Root is Array of Tournaments: { tournament_id, name, country_name, matches: Array<Match> }
// Match is: { match_id, is_finished, ... }

export async function getFlashScoreMatchesRaw(date: Date, sportId: string | number): Promise<any> {
    // Calculate day offset based on LOCAL timezone
    const now = new Date();
    ...
    // Check cache first
    const cacheKey = `matches-list-${safeDay}-${flashScoreSportId}`;
    const cached = memoryCache.get<any>(cacheKey);
    if (cached) {
        return cached;
    }

    const url = `https://${API_HOST}/api/flashscore/v2/matches/list?day=${safeDay}&sport_id=${flashScoreSportId}`;
...
export async function getFlashScoreMatches(date: Date, sportId: string): Promise<Match[]> {
    // Due to timezone differences, matches from a single day in local time
    // might span two days in UTC. Fetch both days to ensure we get all matches.
    const promises = [
        getFlashScoreMatchesRaw(date, sportId),
        // Also fetch next day to catch matches that are in next UTC day but current local day
        getFlashScoreMatchesRaw(new Date(date.getTime() + 24 * 60 * 60 * 1000), sportId)
    ];

    const results = await Promise.all(promises);
    const allData = results.filter(d => d !== null && d !== undefined);

    // Get start and end of target date in LOCAL timezone
    const targetDay = new Date(date.getFullYear(), date.getMonth(), date.getDate());
    const startOfDay = targetDay.getTime();
            ...
            if (tournament.matches && Array.isArray(tournament.matches)) {
                const tournamentMatches = tournament.matches
                    ...
                    .filter((match: Match) => {
                        ...
                        // Filter matches to only include those that fall on the target date in LOCAL timezone
                        const matchTime = match.scheduledAt ? new Date(match.scheduledAt).getTime() : 0;
