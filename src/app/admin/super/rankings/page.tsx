...
export default function SuperRankingsPage() {
    ...
    const selectedRankingSport = useMemo(
        () => canonicalizeSportId(selectedRanking?.sport) || null,
        [selectedRanking?.sport],
    );
    ...
    const catalogClubs = useMemo(
        () => (selectedRankingSport ? catalogBySport[selectedRankingSport] ?? [] : []),
        [catalogBySport, selectedRankingSport],
    );
    const availableEntryClubs = useMemo(() => {
        const usedClubIds = new Set((selectedDetail?.entries ?? []).map((entry) => entry.club_id).filter(Boolean));
        if (entryEditor.mode === 'edit' && entryEditor.clubId) {
            usedClubIds.delete(entryEditor.clubId);
        }
        return catalogClubs.filter((club) => !usedClubIds.has(club.id));
    }, [catalogClubs, entryEditor.clubId, entryEditor.mode, selectedDetail?.entries]);
    ...
    useEffect(() => {
        const sport = selectedRankingSport;
    ...
    useEffect(() => {
        setEntryEditor((current) => {
            ...
            if (!availableEntryClubs.length) {
                return current.clubId ? { ...current, clubId: '' } : current;
            }

            const currentStillAvailable = availableEntryClubs.some((club) => club.id === current.clubId);
            if (currentStillAvailable) {
                return current;
            }

            return { ...current, clubId: availableEntryClubs[0].id };
        });
    }, [availableEntryClubs]);
                            ...
                            {loadingDetailId === selectedRanking?.id && !selectedDetail ? <div className={styles.emptyPreview}>Cargando detalle...</div> : selectedDetail ? <>
                                <div className={styles.tableToolbar}>
                                    ...
                                    <div className={styles.tableToolbarActions}>
                                        <ExportImage className={styles.exportAction} template="standings" filename={`ranking-${selectedRanking?.name || 'clubes'}`} data={{ title: selectedRanking?.name || 'Ranking de Clubes', subtitle: rankingExportSubtitle, rows: rankingExportRows, columnLabels: RANKING_EXPORT_COLUMN_LABELS, plainDiff: true, showPositionDelta: true }} />
                                    </div>
                                </div>
