...
export default function CreateClub() {
    ...
    useEffect(() => {
        ...
        const club = db.clubs.find(c => c.id === clubId && c.unionId === unionId);
        if (!club) return;
        ...
        if (club.logoUrl) {
            setLogoPreview(toDataUrl(club.logoUrl));
        }
    }, [clubId, unionId]);
    ...
    const handlePublish = () => {
        if (isEdit) {
            alert(`Club ${formData.name} actualizado.`);
        } else {
                ...
                <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px' }}>
                    <div>
                        <nav style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px' }} className="mono">
                            ...
                            <span>/</span>
                            <span style={{ color: 'white' }}>{isEdit ? 'Editar Club' : 'Crear Club'}</span>
                        </nav>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            <h1 style={{ fontSize: '30px', fontWeight: 'bold' }}>{isEdit ? 'Editar Club' : 'Nuevo Club'}</h1>
                            <span className="badge-draft">{isEdit ? 'Edicion' : 'Borrador'}</span>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '16px' }}>
                        {currentStep < 5 ? (
                            ...
                            <button
                                ...
                                style={{ padding: '12px 40px', borderRadius: '12px', fontSize: '14px' }}
                            >
                                {isEdit ? 'Guardar cambios' : 'Publicar Club'}
                            </button>
                        )}
                    </div>
                </footer>
