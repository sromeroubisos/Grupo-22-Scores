...
function H2HItem({ m, styles, focusTeamName }: { m: any, styles: any, focusTeamName?: string }) {
    const date = m.timestamp ? new Date(m.timestamp * 1000).toLocaleDateString() : (m.date || '');
            ...
            try {
                if (isFlashScore) {
                    ...
                    const promises: any[] = [
                        ...
                        getFlashScoreMatchesRaw(startTime, sportId),
                    ...
                    const processedMatch = {
                        ...
                        home: { ...baseMatch.home, score: hScoreFinal },
                        away: { ...baseMatch.away, score: aScoreFinal },
                    ...
                    if (localMatch) {
                        ...
                        const processedMatch = {
                            ...
                            time: new Date(localMatch.dateTime).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', hour12: false }),
                ...
                <section className={styles.scoreboardCard}>
                    <div className={styles.scoreboardGrid}>
                        ...
                        <div className={styles.teamCol}>
                                ...
                                {matchData.home.logo ? (
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            // parent element could show placeholder
                                        }}
                                    />
                                ) : (
                            ...
                            <div className={styles.teamInfo}>
                                <div className={styles.teamLabel}>Anfitrión</div>
                                ...
                                <div style={{ marginBottom: '12px' }}>
                                    <ExportImage
                                        ...
                                        data={{
                                            ...
                                            date: new Date(matchData.date).toLocaleDateString('es-AR'),
                                            time: matchData.time,
                    ...
                    <div className={styles.infoBar}>
                        <div className={styles.infoChip}>
                            ...
                            <span><strong>{new Date(matchData.date).toLocaleDateString('es-AR', { day: 'numeric', month: 'short' })}</strong> {matchData.time}</span>
                        </div>
                        <div className={styles.infoChip}>
                                    ...
                                    return (
                                        <div key={i} className={styles.timelineItem}>
                                            ...
                                            <div className={`${styles.eventSide} ${isHome ? styles.eventLeft : styles.eventRight}`}>
                                                ...
                                                <div className={styles.eventDetail}>
                                                    <div className={styles.eventPlayer}>{evt.player}</div>
                                                    {evt.type?.toLowerCase().includes('subst') ? (
                                                        <div className={styles.eventSubInfo}>
                                                            <span className={styles.playerIn}>{evt.player}</span><br />
                                                            <span className={styles.playerOut}>{evt.subPlayer}</span>
                                                        </div>
                                                    ) : (
                                                        <div className={styles.eventSubInfo}>
                                                            {evt.subPlayer && <span className={styles.assistText}>asistencia de {evt.subPlayer}</span>}
                                            ...
                                            <div className={styles.playerList}>
                                                {(matchData.lineups?.HOME_STARTING_LINEUPS || matchData.lineups?.home_team?.starting_lineups || []).map((p: any, i: number) => (
                                                    <div key={i} className={styles.playerItem}>
                                                        <span><span className={styles.playerNumber}>{p.PLAYER_NUMBER || p.player_number}</span> {p.PLAYER_NAME || p.player_name}</span>
                                                ...
                                                {matchData.standings.slice(0, 20).map((row: any, i: number) => {
                                                    ...
                                                    return (
                                                        <tr key={i} className={isCurrent ? styles.currentTeam : ''}>
                                                            ...
                                                            <td style={isCurrent ? { color: 'var(--accent)', fontWeight: '700' } : {}}>{rowName}</td>
                                                    ...
                                                    {group.stats.map((s: any, j: number) => (
                                                        <div key={j} className={styles.playerStatRow}>
                                                            <div className={styles.playerStatHome}>
                                                                {s.home_team && (
                                                                    <>
                                                                        <span className={styles.playerStatName}>{state.playerStats.players?.find((p: any) => p.player_id === s.home_team.player_id)?.player_name || 'Jugador'}</span>
                                                            ...
                                                            <div className={styles.playerStatAway}>
                                                                {s.away_team && (
                                                                    <>
                                                                        ...
                                                                        <span className={styles.playerStatName}>{state.playerStats.players?.find((p: any) => p.player_id === s.away_team.player_id)?.player_name || 'Jugador'}</span>
                                                                    </>
                                                                )}
                                                            </div>
                                                        </div>
