...
import {
  ...
  parseResultsTournamentSearchPayload,
  searchResultsTournaments,
  toResultsApiError,
...
export async function POST(request: NextRequest) {
  ...
  try {
    const payload = parseResultsTournamentSearchPayload(await request.json().catch(() => null));
    const response = await searchResultsTournaments(payload);
