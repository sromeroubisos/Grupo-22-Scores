...
async function resolveIdsFromTournamentId(tournamentId: string, sportId: string = 'football'): Promise<ResolvedIds> {
    ...
    for (const offset of offsets) {
        ...
        try {
            const raw = await getFlashScoreMatchesRaw(d, sportId);
...
export async function GET(request: Request) {
    ...
    try {
        ...
        const finalDetails = normalizeDetails(detailsRes) || details;

        console.log('FINAL RESOLVED:', {
            tournamentId, stageId, templateId, seasonId,
            hasDetails: !!finalDetails,
            results: resultsRes?.DATA?.length || resultsRes?.length || 0,
            standings: standingsRes?.DATA?.length || standingsRes?.length || 0
        });

        // --- Custom Re-grouping Logic ---
        let finalStandings = standingsRes?.DATA || standingsRes || [];
        ...
        return Response.json({
            ...
            _debug: {
                ...
                counts: {
                    results: resultsRes?.DATA?.length || resultsRes?.length || 0,
                    fixtures: fixturesRes?.DATA?.length || fixturesRes?.length || 0,
                    standings: finalStandings.length,
                    topScorers: topScorersRes?.DATA?.length || topScorersRes?.length || 0
                }
            },
            ids: { tournamentId, stageId, templateId, seasonId, drawStageId },
            details: finalDetails,
            results: resultsRes?.DATA || resultsRes || [],
            fixtures: fixturesRes?.DATA || fixturesRes || [],
            standings: finalStandings,
            standingsForm: standingsFormRes?.DATA || standingsFormRes || [],
            standingsHtFt: standingsHtFtRes?.DATA || standingsHtFtRes || [],
            standingsOverUnder: standingsOverUnderRes?.DATA || standingsOverUnderRes || [],
            topScorers: topScorersRes?.DATA || topScorersRes || [],
            draw: drawRes?.DATA || drawRes || [],
            archives: archivesRes?.DATA || archivesRes || []
        });
    } catch (e: any) {
