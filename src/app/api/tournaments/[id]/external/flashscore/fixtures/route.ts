...
function normalizeMatches(raw: any): Array<{ id: string; home: string; away: string; ts: number; venue?: string }> {
    const matches: Array<{ id: string; home: string; away: string; ts: number; venue?: string }> = [];
    ...
    for (const item of data) {
        ...
        // Could be grouped by tournament like match list: [{tournament_id, matches: [...]}]
...
function extractMatch(m: any): { id: string; home: string; away: string; ts: number; venue?: string } | null {
    ...
    return { id: String(id), home: String(home), away: String(away), ts: Number(ts), venue: m.venue ?? undefined };
}
...
export async function GET(
    ...
    try {
        ...
        const raw = await getTournamentFixtures(
        ...
        if (!raw) {
            return NextResponse.json(
                { error: 'No se pudo obtener el fixture de FlashScore. Intentá nuevamente.' },
                { status: 502 }
            );
        }
