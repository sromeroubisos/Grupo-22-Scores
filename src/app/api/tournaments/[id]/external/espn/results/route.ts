...
import { getEspnAmericanFootballLeagueResults, isEspnAmericanFootballLeagueSlug } from '@/lib/services/espnAmericanFootball';
import { getEspnMotorsportLeagueResults, isEspnMotorsportLeagueSlug } from '@/lib/services/espnMotorsport';
...
export async function GET(
    ...
    try {
        ...
        const results = isAmericanFootball
            ? await getEspnAmericanFootballLeagueResults(config!.league_slug as any, page)
            : await getEspnMotorsportLeagueResults(config!.league_slug as any, page);
        const matches = results.matches
            .filter((match): match is NonNullable<(typeof results.matches)[number]> => Boolean(match))
            .map((match) => {
